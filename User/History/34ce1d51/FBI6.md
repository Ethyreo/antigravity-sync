...
## Phase 8: Master Cloud Database Integration
- [ ] Design structural Google Sheets tabs for persistence (if needed beyond what exists)
- [ ] Build `netlify/functions/fetchData.js` to query Google Sheets
- [ ] Implement asynchronous Database context wrapper in `App.jsx` with a beautiful Loading Screen
- [ ] Override the empty `buildingLayout.js` skeleton with LIVE cloud data
- [ ] Refactor all submit actions to perform CRUD straight to the cloud API instead of purely local state
