...
- [ ] Fix the "Paid" vs "Unknown" default state drift bug in Bulk Entry UI
- [ ] Upgrade the `buildingLayout.js` Data Model with nested `tenantHistory` lists (Name, Company, Join Date, Leave Date)
- [ ] Create `HistoricalTenants.jsx` component (Table View with search/filters)
- [ ] Add navigation routing and a dedicated button beside "Bulk Data" on the Home view
- [ ] Upgrade the Netlify Serverless Function `sync.js` to additionally build and sync a "Tenant Directory" tab in Google Sheets
