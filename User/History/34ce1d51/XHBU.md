...
- [ ] Upgrade the global `isOccupied` function logic to properly evaluate `leaveDate >= today()` for future-dated evictions/handovers.
- [ ] Overhaul `TenantManagementModal.jsx` into three distinct modes: Add, Replace, Evict.
- [ ] Implement robust date-coordination logic for "Replace" (outgoing Leave Date + incoming Join Date).
- [ ] Update `cloudSync.js` to correctly determine the "Active Tenant" for the current month's Ledger based on the overlapping date boundaries.
