...
- [x] Refactor `BulkDataEntry.jsx` to use a per-row `includesUtilities` checkbox.
- [x] Add a confirmation dialog in `BulkDataEntry.jsx` when saving data for a month prior to the current billing cycle.
- [x] Refactor `Dashboard.jsx` to replace existing charts with a Water/Electricity dual-line chart tracking from Jan 2026.
- [x] Ensure all `Dashboard.jsx` top-level KPI cards accurately react to the selected `FilterBar` context.
- [x] Update the default local state to anchor the application ledger and dashboard analytics to Jan 2026.

## Phase 17: Granular Billing Controls & Overwrite Verification (Completed)
- [x] Add explicit "Includes Utilities" and "Custom Rent" checkboxes to each row in `BulkDataEntry.jsx`.
- [x] Wire the "Custom Rent" checkbox to toggle the Rent Display into an editable `<input>` field.
- [x] Implement `OverwriteWarningModal.jsx` (integrated) to intercept historical month saves in the Bulk entry screen.
- [x] Within the warning modal, query Firestore to pull the *current* saved snapshot of that month's ledger, displaying the timestamp and old values to the user before they commit the overwrite.

## Phase 18: Ledger Upgrades (PDF & Multi-Select Filters) (Completed)
- [x] Refactor `LedgerTable.jsx` to change filter states (`Month`, `Status`, `Type`) from single strings to Arrays.
- [x] Build an inline `MultiSelectDropdown` component featuring a popover list of checkboxes to replace native `<select>` tags without bloating dependencies.
- [x] Ensure the Filter logic correctly searches against the active arrays.
- [x] Fix the `Export PDF` function by directly importing and calling `autoTable(doc, config)` to bypass Vite's prototype mangling.

## Phase 19: Global Dashboard Multi-Select Controls (Completed)
- [x] Refactor `LedgerTable.jsx` to completely delete the "Type" filter (Residential vs Commercial) as the building is entirely Residential.
- [x] Update `FilterContext.jsx` so its default state tracks arrays (`{ floor: [], unit: [] }`) instead of strings (`'all'`).
- [x] Rewrite `FilterBar.jsx` to swap out native `<select>` dropdowns for custom `MultiSelectPopover` components.
- [x] Refactor the filter ingestion logic (`.includes()`) in `Dashboard.jsx`.
- [x] Refactor the filter ingestion logic (`.includes()`) in `BuildingVisualMap.jsx`.

## Phase 20: Codebase Refactor & Optimization (Completed)
- [x] Sweep for and remove unused variables and imports.
- [x] Remove development `console.log` debugging statements.
- [x] Clean up deprecated or heavily commented-out legacy code blocks.

## Phase 21: Rent Receipt PDF Generator (Completed)
- [x] Create `RentReceiptGenerator.jsx` with tenant/month selection and payment-status gating.
- [x] Build professional A4 PDF receipt with property address, owner details, billing breakup, amount in words, and signature lines.
- [x] Wire into `App.jsx` with a new "Rent Receipt" navigation button and `receipt` view route.
- [x] Deploy to Firebase Hosting and push to GitHub.
