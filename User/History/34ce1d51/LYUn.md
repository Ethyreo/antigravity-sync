...
## Phase 17: Granular Billing Controls & Overwrite Verification (Active)
- [ ] Add explicit "Includes Utilities" and "Custom Rent" checkboxes to each row in `BulkDataEntry.jsx`.
- [ ] Wire the "Custom Rent" checkbox to toggle the Rent Display into an editable `<input>` field.
- [ ] Implement `OverwriteWarningModal.jsx` to intercept historical month saves in the Bulk entry screen.
- [ ] Within the warning modal, query Firestore to pull the *current* saved snapshot of that month's ledger, displaying the timestamp and old values to the user before they commit the overwrite.
