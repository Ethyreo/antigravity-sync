...
## Phase 18: Ledger Upgrades (PDF & Multi-Select Filters) (Active)
- [ ] Refactor `LedgerTable.jsx` to change filter states (`Month`, `Status`, `Type`) from single strings to Arrays.
- [ ] Build an inline `MultiSelectDropdown` component featuring a popover list of checkboxes to replace native `<select>` tags without bloating dependencies.
- [ ] Ensure the Filter logic correctly searches against the active arrays.
- [ ] Fix the `Export PDF` function by directly importing and calling `autoTable(doc, config)` to bypass Vite's prototype mangling.
