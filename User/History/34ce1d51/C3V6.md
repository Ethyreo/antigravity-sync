...
- [ ] Install `jspdf` and `jspdf-autotable` dependencies for PDF generation
- [ ] Create `LedgerTable.jsx` component representing the raw Firebase tabular output
- [ ] Build a filter UI schema (Month, Status, Unit Type) mapping to the table payload
- [ ] Code CSV/Excel raw text Blob export utility hook
- [ ] Code PDF structured table export utility hook
- [ ] Integrate `LedgerTable` into `App.jsx` UI Flow (Beneath the Map/Dashboard)
