...
## Phase 9: Firebase Architecture Migration (Active)
- [ ] Receive `firebaseConfig` object from the user
- [ ] Install `firebase` SDK and initialize `src/config/firebase.js`
- [ ] Rip out Netlify Serverless Functions (`sync.js`, `fetchData.js`) and `google-spreadsheet`
- [ ] Rewrite `cloudSync.js` to push and pull from Cloud Firestore collections (`buildingLayout`, `ledger`, `tenantHistory`)
- [ ] Migrate `AuthGate.jsx` to natively use Firebase Authentication or unified Firestore secret key
- [ ] Implement robust NoSQL data hydration loop in `App.jsx` bypassing Vite HTTP Proxies entirely
