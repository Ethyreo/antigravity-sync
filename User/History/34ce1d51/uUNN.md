...
- [ ] Refactor `LedgerTable.jsx` to completely delete the "Type" filter (Residential vs Commercial) as the building is entirely Residential.
- [ ] Update `FilterContext.jsx` so its default state tracks arrays (`{ floor: [], unit: [] }`) instead of strings (`'all'`).
- [ ] Rewrite `FilterBar.jsx` to swap out native `<select>` dropdowns for custom `MultiSelectPopover` components.
- [ ] Refactor the filter ingestion logic (`.includes()`) in `Dashboard.jsx`.
- [ ] Refactor the filter ingestion logic (`.includes()`) in `BuildingVisualMap.jsx`.
