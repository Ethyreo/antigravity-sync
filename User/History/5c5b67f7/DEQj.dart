...
class _SnakePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    if (state.snake.isEmpty) return; // Prevent crash on optional empty
    ...
    for (int i = 0; i < state.snake.length; i++) {
      ...
      paint.color = CyberColors.electricCyan.withOpacity(opacity);
  ...
  bool shouldRepaint(covariant _SnakePainter oldDelegate) {
    // Simple equality check isn't enough for deep object, but state is immutable copy
    return true; 
  }
}
