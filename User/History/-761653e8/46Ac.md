...
### Phase 5 Features Added (Google Sheets Live Integration)
*   **Secure API Proxy Server:** To prevent exposing the Google Service Account private key in the frontend client code (which is a massive security risk in React apps), engineered a custom local `Express.js` backend server (`server.js`). This node layer securely holds the credentials and acts as a gateway proxy.
*   **Dual-Boot Architecture:** Integrated the `concurrently` package so that running `npm run dev` now seamlessly launches *both* the Vite frontend and the secure Node payload-processor backend at once on ports `5180` and `3001`.
*   **The Live Sync Endpoint:** When you hit "Save All" on the Bulk Update grid, it now pushes a fully assembled HTTP POST payload to the Node proxy.
*   **Smart Ledger Management:** The backend authenticates via the `google-auth-library`. It then seeks out your target spreadsheet. If it cannot find a sheet named "Oak Lodge Monthly Ledger", it instantly creates one and formats the column headers automatically. It then clears any old/stale entries for the target month, and bulk-appends the fresh data grid straight from your UI into the cloud in real time.
