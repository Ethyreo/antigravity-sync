...
## Phase 10: Interactive Ledger Table & PDF Export
To replace the raw Google Sheets backend viewing experience, a native unified Data Table was integrated directly beneath the visual map on the main dashboard.

### Core Features
1. **Real-time Firestore Hydration**: The table (`LedgerTable.jsx`) memoizes the identical `cloudDb.ledger` array retrieved during the initial app load, ensuring zero extra latency or API calls.
2. **Dense Filtering Engine**: Users can dynamically filter the raw thousands of rows by:
   - Specific **Billing Month** (Descending list dynamically generated from the active database).
   - **Rent Status** (Paid, Unpaid, Overdue).
   - **Unit Type** (Commercial or Residential, cross-referenced dynamically against the static config layout).
   - Global **Fuzzy Search** across Tenant Name, Unit ID, and Unit Name.
3. **Data Aggregation**: The table instantly calculates live sums for Rent Collected, Electricity, Water, and Garbage specifically for the active filtered scope, rendering them in a highlighted footer row.
4. **Offline Export Generation**: 
   - **CSV / Excel:** Formats the active view into a comma-separated `.csv` blob natively forced to download within the browser.
   - **PDF Rendering:** Utilizes `jspdf` and `jspdf-autotable` to draw strict, beautifully formatted invoice-style PDF documents directly from the filtered view. Employs conditional cell coloring (Red for Unpaid, Green for Paid) translated perfectly into the printable document.
