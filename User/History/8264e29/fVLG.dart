  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        return Stack(
          children: [
            // The Game Page (Child) - Revealed by mask
            ClipRect(
              child: ShaderMask(
                shaderCallback: (rect) {
                  final center = focalPoint ?? rect.center;
                  final maxDist = sqrt(rect.width * rect.width + rect.height * rect.height);
                  final radius = animation.value * maxDist * 1.5; // Expand well past screen
                  ...
                  return RadialGradient(
                    ...
                    radius: radius / rect.shortestSide, // Normalized radius
                    colors: const [Colors.white, Colors.transparent],
                    stops: const [0.0, 1.0], // Hard edge or smooth? Smooth looks better for "gradient reveal"
                    // Actually, for "revealing", we want the child to be visible where the shader is WHITE (if dstIn)
                    // or invisible where Black (if srcOut).
                    // Default logic: ShaderMask applies shader color to child. 
                    // Use BlendMode.dstIn: 
                    // The destination (child) shows where the source (shader) is opaque.
                    // So we want Opaque Center -> Transparent Edges (to hide edges).
                  ).createShader(rect);
                },
                blendMode: BlendMode.dstIn,
                child: Container(
                  color: CyberColors.voidBlack, // Ensure background is filled
                  child: child,
                ),
              ),
            ),
            
            // The Decoration Lines (Overlay)
