...
const LedgerTable = ({ ledgerData }) => {
    ...
    const [openDropdown, setOpenDropdown] = useState(null); // 'month' | 'status' | 'type' | null
    ...
    const filteredData = useMemo(() => {
        return enrichedData.filter(row => {
            ...
            const matchType = filterTypes.length === 0 || filterTypes.includes(row.UnitType.toLowerCase());
    ...
    }, [enrichedData, filterMonths, filterStatuses, filterTypes, searchQuery]);
    ...
    const handleExportPDF = () => {
        ...
        const tStr = filterTypes.length ? filterTypes.join(', ') : 'All';

        doc.text(`Generated: ${new Date().toLocaleDateString()} | Months: ${mStr} | Statuses: ${sStr} | Types: ${tStr}`, 14, 30);
