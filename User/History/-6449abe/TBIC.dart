...
class _BrandMatchesScreenState extends State<BrandMatchesScreen>
  ...
  List<Match> _matches = [];
  Map<String, CreatorProfile> _creatorCache = {};
  ...
  Future<void> _loadData() async {
    ...
    // 1. Fetch matches
    final matches = await _matchingService.matchesForBrand(_brandId);
    
    // 2. Fetch creators for these matches
    // Note: optimization - we could fetch all creators once or batch
    final creators = await _supabaseService.getCreators(); 
    final creatorMap = {for (var c in creators) c.id: c};

    if (mounted) {
      setState(() {
        _matches = matches;
        _creatorCache = creatorMap;
        _isLoading = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
            // ... (rest of the body is fine as it uses local _matches list)
      body: TabBarView(
        controller: _tabController,
        children: [
          // ── Pending Tab ──
