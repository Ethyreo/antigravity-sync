...
- [x] Implement `CyberCell` rendering box (Cyan outlines, dark fill, hazard colors)
- [x] Implement `game.dart` view using `GridView.builder` or `InteractiveViewer`
- [x] Implement Cyber-style Header (Mines counter, retro fonts, toggles)
- [x] Implement Control scheme (Tap = Reveal, LongPress/Toggle = Flag)
- [x] Register game into `GameRegistry` and `routes.dart`
