...
/**
 * Validates if data exists for a given month before overwriting.
 * Used by the Historical Overwrite Warning Modal in BulkDataEntry.
 */
export const fetchLedgerMonthSnapshot = async (month) => {
    try {
        const q = query(collection(db, "Ledger"), where("BillingMonth", "==", month));
        const snap = await getDocs(q);
        const records = [];
        snap.forEach(doc => records.push(doc.data()));
        return { success: true, exists: records.length > 0, data: records };
    } catch (err) {
        console.error("Critical Cloud Snapshot Error:", err);
        return { success: false, exists: false, data: [] };
    }
};
