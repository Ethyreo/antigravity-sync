...
function App() {
  ...
  const [ledgerDatabase, setLedgerDatabase] = useState([]); // Phase 10: Master Table Array
  ...
  const hydrateAppFromCloud = async () => {
    ...
    try {
      ...
      setLedgerDatabase(cloudDb.ledger || []);
      ...
      buildingData.floors.forEach(floor => {
        floor.units.forEach(unit => {
          ...
          // 0. Map Hard Config (Meters, Types) from Units Payload
          const unitConfig = cloudDb.units?.find(row => row.UnitID === unit.id);
          if (unitConfig) {
            unit.elecUnit = unitConfig.ElectricityMeter || unit.elecUnit;
            unit.waterConn = unitConfig.WaterMeter || unit.waterConn;
            unit.garbageId = unitConfig.GarbageId || unit.garbageId;
          }
        ...
        <main className="pt-24 px-4 sm:px-6 max-w-5xl mx-auto">
          {currentView === 'home' ? (
            <>
              ...
              {/* Phase 10: Interactive Ledger Table */}
              <div className="mt-12">
                <LedgerTable ledgerData={ledgerDatabase} />
              </div>
            </>
          ) : currentView === 'bulk' ? (
            <BulkDataEntry
