...
// ─── Constants ────────────────────────────────────────
class HC {
  static const Color sky1 = Color(0xFF0a0a2e);
  static const Color sky2 = Color(0xFF000010);
  static const Color ground = Color(0xFF0d2b0d);
  static const Color groundTop = Color(0xFF00FF88);
  static const Color platEdge = Color(0xFF00FFFF);
  static const Color platFill = Color(0xFF0a1a2a);
  static const Color playerBody = Color(0xFF00E0E0);
  static const Color playerSpike = Color(0xFF007090);
  static const Color playerShoe = Color(0xFFFF0066);
  static const Color enemyBody = Color(0xFFFF00FF);
  static const Color enemyEye = Color(0xFFFFFFFF);
  static const Color ringOuter = Color(0xFF00FFFF);
  static const Color ringInner = Color(0xFFFFF000);
  static const Color goalCyan = Color(0xFF00FFFF);
  static const Color star = Color(0x88FFFFFF);
  static const Color hillFar = Color(0xFF061828);
  static const Color hillNear = Color(0xFF0a2838);
}
...
class HedgehogState {
  ...
  PlayerState pState;
  FacingDir facing;
  double hurtTimer, respawnTimer, animTime, stateTime;
  bool grounded;
  ...
  HedgehogState()
      : px = 50, py = 180, vx = 0, vy = 0,
        ...
        facing = FacingDir.right,
        hurtTimer = 0, respawnTimer = 0, animTime = 0, stateTime = 0,
        grounded = false,
...
class HedgehogEngine {
  ...
  void update(double dt) {
    ...
    // Running dust
    if (state.grounded && state.vx.abs() > 50) {
      state.particles.emitRunDust(
        state.px + GC.playerW / 2, 
        state.py + GC.playerH, 
        state.vx.sign
      );
    }
  ...
  void _input(double dt) {
    ...
    if (state.inJump && state.grounded) {
      ...
      state.inJump = false;
      // Emit jump sparks
      state.particles.emitJumpSparks(state.px + GC.playerW/2, state.py + GC.playerH);
    }
  }
  ...
  void _collide() {
    state.grounded = false;
    final rP = R2(state.px, state.py, GC.playerW, GC.playerH);

    for (final p in state.platforms) {
      if (!rP.hits(p.rect)) continue;

      final oL = rP.r - p.rect.x;
      final oR = p.rect.r - rP.x;
      final oT = rP.b - p.rect.y;
      final oB = p.rect.b - rP.y;
      double m = [oL, oR, oT, oB].reduce(min);

      // Fix "getting stuck randomly" edge catching:
      // If falling onto the corner of a platform, vertical penetration might be larger 
      // than horizontal due to fast terminal velocity frame timesteps. 
      // Force it to register as a top landing rather than pushing them sideways off the ledge.
      if (state.vy >= 0 && oT <= 16 && (m == oL || m == oR)) {
        m = oT;
      }
      ...
      // Update our checking rect in case we resolved a collision, 
      // preventing false overlap detections on adjacent blocks.
      rP.x = state.px;
      rP.y = state.py;
    }
    ...
    if (state.pState != PlayerState.hurt) {
      ...
      } else if (state.vx.abs() > 10 || state.inLeft || state.inRight) {
    ...
    if (state.hurtTimer <= 0) {
      ...
      for (final e in state.enemies) {
        ...
        if (landedOn) {
          ...
          // Emit explosion
          state.particles.emitExplosion(e.rect.cx, e.rect.cy, HC.enemyBody, 12);
        } else {
          _hurt();
        }
      }
