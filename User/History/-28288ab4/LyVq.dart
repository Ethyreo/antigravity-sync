  ...
  void _drawEnemies(Canvas c) {
    for (final e in s.enemies) {
      ...
      if (e.type == EnemyType.walker) {
        ...
        // Body dome
        c.drawArc(
            Rect.fromLTWH(r.x, r.y + 8, r.w, r.h - 8), 
            pi, pi, true, 
            Paint()..color = bodyColor
        );
        c.drawRect(Rect.fromLTWH(r.x, r.cy + 4, r.w, 4), Paint()..color = bodyColor.withOpacity(0.7));

        // Eye
        final eyeX = e.facing == FacingDir.right ? r.x + 16 : r.x + 6;
        c.drawCircle(Offset(eyeX, r.y + 14), 3, Paint()..color = eyeColor);
        c.drawCircle(Offset(eyeX, r.y + 14), 1.5, Paint()..color = Colors.black);

        // Legs
        final legShift = sin(e.t * 15) * 4;
        final legPaint = Paint()..color = bodyColor..strokeWidth = 2..style = PaintingStyle.stroke;
        c.drawLine(Offset(r.x + 4, r.cy + 8), Offset(r.x + 2 - legShift, r.b), legPaint);
        c.drawLine(Offset(r.x + 13, r.cy + 8), Offset(r.x + 13 + (legShift * 0.5), r.b), legPaint);
        c.drawLine(Offset(r.x + 22, r.cy + 8), Offset(r.x + 24 + legShift, r.b), legPaint);

      } else {
        // Draw Flying Saucer
        // Glow thrust
        final thrust = sin(e.t * 20).abs() * 6;
        c.drawArc(Rect.fromLTWH(r.x + 6, r.b - 8 - thrust, r.w - 12, 16 + thrust), 0, pi, true, Paint()..color = Colors.cyanAccent.withOpacity(0.6));
        
        // Saucer rim
        c.drawOval(Rect.fromLTWH(r.x - 4, r.cy, r.w + 8, 10), Paint()..color = HC.enemyBody);
        
        // Glass dome
        c.drawArc(Rect.fromLTWH(r.x + 2, r.y + 4, r.w - 4, 20), pi, pi, true, Paint()..color = HC.enemyEye.withOpacity(0.7));
        
        // Blinking light
        if ((e.t * 5).floor() % 2 == 0) {
          c.drawCircle(Offset(r.cx, r.y + 6), 2, Paint()..color = Colors.redAccent);
        }
      }
    }
  }
