...
class HedgehogPainter extends CustomPainter {
  ...
  void _drawSky(Canvas c) {
    final paint = Paint()
      ...
      ).createShader(const Rect.fromLTWH(0, 0, GC.viewW, GC.viewH));
    c.drawRect(const Rect.fromLTWH(0, 0, GC.viewW, GC.viewH), paint);

    // Stars
    final starPaint = Paint()..color = HC.star;
    final rng = Random(42);
    for (int i = 0; i < 40; i++) {
      final x = rng.nextDouble() * GC.viewW;
      final y = rng.nextDouble() * 160;
      final r = 0.5 + rng.nextDouble() * 1.0;
      final flicker = (sin(s.animTime * 2 + i) + 1) / 2;
      starPaint.color = HC.star.withOpacity(0.3 + flicker * 0.5);
      c.drawCircle(Offset(x, y), r, starPaint);
    }
  }

  // ── Parallax Hills ──
  void _drawHills(Canvas c, double cam) {
    final p = Paint();
    // Far hills
    p.color = HC.hillFar;
    _drawHillLayer(c, p, cam * 0.15, 180, 70, 200, 7);
    // Near hills
    p.color = HC.hillNear;
    _drawHillLayer(c, p, cam * 0.3, 200, 50, 140, 9);
  }

  void _drawHillLayer(
      Canvas c, Paint p, double offset, double baseY, double h,
      double w, int count) {
    for (int i = 0; i < count; i++) {
      final x = i * w * 0.7 - (offset % (w * 0.7 * count));
      final path = Path()
        ..moveTo(x, baseY + 40)
        ..quadraticBezierTo(x + w / 2, baseY - h, x + w, baseY + 40)
        ..close();
      c.drawPath(path, p);
    }
  }
