...
export class Player {
    ...
    render(ctx) {
        ...
        if (Math.abs(this.velocityX) > 6 && this.grounded) {
            this.renderSpeedLines(ctx);
        }

        // After-image effect when spinning fast
        if ((this.state === 'spinning' || this.isSuper) && Math.abs(this.velocityX) > 4) {
        ...
        if (this.isSuper) {
            ...
            // Aura particles
            this.renderSuperAura(ctx);
        }

        // Invincibility sparkles
        if (this.invincible && !this.isSuper && this.invincibilityTimer > 60) {
            this.renderInvincibilitySparkles(ctx);
        }

        // Shield effect
        if (this.hasShield && !this.isSuper) {
            const time = Date.now() * 0.003;
            const radius = Math.max(this.width, this.height) / 2 + 6 + Math.sin(time * 3) * 2;
            ...
            shieldGrad.addColorStop(0, 'rgba(100, 200, 255, 0.1)');
            shieldGrad.addColorStop(0.8, 'rgba(100, 200, 255, 0.2)');
        ...
        if (this.sprite) {
            ...
            // Enhanced fallback - Sonic silhouette
            this.renderSonicFallback(ctx);
        }
    ...
    renderSpeedLines(ctx) {
        ...
        const numLines = Math.floor(speed / 2);

        ctx.strokeStyle = 'rgba(255, 255, 255, 0.4)';
        ctx.lineWidth = 1;

        for (let i = 0; i < numLines; i++) {
            const lineY = this.y + 5 + (this.height - 10) * (i / numLines);
            const lineLength = 10 + speed * 2;
            const startX = this.facing > 0 ? this.x - lineLength : this.x + this.width;

            ctx.beginPath();
            ctx.moveTo(startX, lineY);
            ctx.lineTo(startX + lineLength * this.facing * -1, lineY);
            ctx.stroke();
        }
    }

    /**
     * Render after-image effect
     */
    renderAfterImages(ctx) {
        ...
        for (let i = 1; i <= images; i++) {
            ...
            const offsetX = -this.facing * i * spacing;

            ctx.globalAlpha = alpha;

            if (this.sprite) {
                ctx.drawImage(this.sprite, this.x + offsetX, this.y, this.width, this.height);
            } else {
    ...
    renderSuperAura(ctx) {
        ...
        // Rotating particles around Sonic
        for (let i = 0; i < 6; i++) {
            ...
            ctx.fillStyle = `rgba(255, 215, 0, ${0.5 + Math.sin(time + i) * 0.3})`;
            ctx.beginPath();
    ...
    /**
     * Render invincibility sparkles
     */
    renderInvincibilitySparkles(ctx) {
    ...
    /**
     * Render Sonic fallback (no sprite)
     */
    renderSonicFallback(ctx) {
        const x = this.x;
        const y = this.y;
        const w = this.width;
        const h = this.height;
        ...
        // Body color
        const bodyColor = this.isSuper ? '#FFD700' : '#1565C0';
        ...
        ctx.fillStyle = bodyColor;
        for (let i = 0; i < 3; i++) {
            const spikeX = x + w * 0.4 - i * 5;
            const spikeY = y + h * 0.2 + i * 3;
            ctx.beginPath();
            ctx.moveTo(spikeX, spikeY);
            ctx.lineTo(spikeX - 12, spikeY - 4);
            ctx.lineTo(spikeX, spikeY + 4);
            ctx.fill();
        }
        ...
        ctx.beginPath();
        ctx.arc(x + w * 0.55, y + h * 0.35, w * 0.15, 0, Math.PI * 2);
        ctx.fill();

        // Eye
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(x + w * 0.58, y + h * 0.32, 3, 0, Math.PI * 2);
        ctx.fill();

        // Eye highlight
        ctx.fillStyle = '#fff';
        ctx.fillRect(x + w * 0.57, y + h * 0.30, 2, 2);
