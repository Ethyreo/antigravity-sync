...
import { getCurrentRentAmount, calculateTotalCollectable } from '../utils/rentUtils';
    ...
    const handleMassSave = async () => {
        ...
        buildingData.floors.forEach(floor => {
            floor.units.forEach(unit => {
                ...
                // Add to Google Sheets upload payload
                payloadRows.push({
                    BillingMonth: selectedMonth,
                    UnitID: unit.id,
                    UnitName: unit.name,
                    RentStatus: draft.rentStatus,
                    RentAmount: draft.rentAmount,
                    Electricity: Number(draft.elecBill) || 0,
                    Water: Number(draft.waterBill) || 0,
                    Garbage: Number(draft.garbageBill) || 0,
                    TotalCollectable: Number(draft.rentAmount) + Number(draft.elecBill) + Number(draft.waterBill) + Number(draft.garbageBill)
                });
            });
        });

        // Trigger local UI re-render instantly
        triggerUpdate();

        // Push to Netlify Serverless Function
        try {
