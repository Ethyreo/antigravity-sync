...
export default function BulkDataEntry({ onBack, triggerUpdate }) {
    ...
    const [includesUtilities, setIncludesUtilities] = useState(false); // Global toggle for rent-inclusive utilities
    ...
    // Auto-zero utilities if checkbox toggled
    React.useEffect(() => {
    ...
    return (
        <div className="bg-white border border-slate-200 rounded-[2rem] p-6 sm:p-8 mt-8 shadow-sm relative overflow-hidden">
            ...
            {/* Utility Toggle */}
            <div className="flex items-center gap-3 mb-6 bg-blue-50/50 border border-blue-100 rounded-xl p-3 shrink-0 self-start sm:self-auto">
                <input
                    type="checkbox"
                    id="includeUtilities"
                ...
                />
                <label htmlFor="includeUtilities" className="text-sm font-bold text-blue-900 cursor-pointer select-none">
                    Rent Includes Garbage & Water
                </label>
            </div>

            <div className="flex justify-end mb-6 w-full sm:w-auto">
                <motion.button
                                ...
                                <div key={unit.id} className="grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_1fr_1fr] gap-3 items-center hover:bg-slate-50 p-2 -mx-2 rounded-xl transition-colors">
                                    ...
                                    <div>
                                        <input
                                            ...
                                            className={clsx(
                                                ...
                                                includesUtilities ? "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed" : "bg-white border-slate-200 focus:border-blue-400 text-slate-800"
                                            )}
                                        />
                                    </div>
                                    ...
                                    <div>
                                        <input
                                            ...
                                            className={clsx(
                                                "w-full border rounded-lg px-3 py-2 text-sm font-bold text-center outline-none transition-colors",
                                                includesUtilities ? "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed" : "bg-white border-slate-200 focus:border-slate-800 text-slate-800"
                                            )}
                                        />
                                    </div>
