...
export default function BulkDataEntry({ onBack, triggerUpdate }) {
    ...
    const handleMassSave = async () => {
        // --- PHASE 16: HISTORICAL EDIT GUARDRAIL ---
        // Validate if the selected month is strictly older than the real-world current month
        const thisMonthStr = getCurrentBillingMonth(); // e.g. "2026-02"
        if (selectedMonth < thisMonthStr) {
            const confirmed = window.confirm(`WARNING: You are about to overwrite Historical Ledger Data for ${selectedMonth}.\n\nAre you sure you want to retroactively alter the financial records for a past month?`);
            if (!confirmed) return; // User aborted
        }
                                ...
                                <div key={unit.id} className="grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_1fr_1fr] gap-3 items-center hover:bg-slate-50 p-2 -mx-2 rounded-xl transition-colors">
                                    ...
                                    <div className="relative group">
                                        <input
                                            ...
                                            className={clsx(
                                                "w-full border rounded-lg px-3 py-2 text-sm font-bold text-center outline-none transition-colors",
                                        ...
                                        {/* Hidden Utilities Toggle Switch triggerable by right-clicking or a small icon */}
                                        <button
                                            title="Toggle if Rent Includes Water & Garbage"
                                            onClick={() => handleCellChange(unit.id, 'includesUtilities', !draft.includesUtilities)}
                                            className="absolute -right-2 -top-2 bg-white border rounded-full p-1 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity hover:bg-slate-50"
                                        >
                                            <div className={clsx("w-2 h-2 rounded-full", draft.includesUtilities ? "bg-blue-500" : "bg-slate-300")} />
                                        </button>
                                    </div>
