...
### Phase 8: Pure Cloud Architecture (Google Sheets Hydration)
- **Source of Truth Shift:** Currently, the app reads locally and pushes to Google Sheets. We must rip this out and invert the flow. Google Sheets is now the master Database.
- **Read APIs:** Create a new Serverless Function (`netlify/functions/fetchData.js`) that queries the Google Sheets API precisely on App startup to pull down the Monthly Ledger tab and the Tenant Directory tab.
- **Dynamic Hydration:** Modify `App.jsx` to show a Loading Screen. Once the API returns the arrays, dynamically stitch the cloud data into the blank physical structure imported from `buildingLayout.js`.
- **Full Sync Loops:** All mutations (Adding single tenants, Evicting, or Mass Updating Ledger) must instantly POST to the Google Sheets backend, and optionally re-trigger a GET to keep the frontend completely synchronized with the cloud.
