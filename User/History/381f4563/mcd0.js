...
export const handler = async (event, context) => {
    ...
    try {
        ...
        const { month, rows: payloadRows, tenantRows, configRows } = payload;
        ...
        if (!sheet) {
            sheet = await doc.addSheet({
                ...
                headerValues: ['BillingMonth', 'UnitID', 'UnitName', 'TenantName', 'RentStatus', 'RentAmount', 'Electricity', 'Water', 'Garbage', 'TotalCollectable']
            });
        }
        ...
        // --- 3. Synchronize Master Configurations ---
        if (configRows && configRows.length > 0) {
            console.log(`[Netlify Serverless] Syncing Master Configurations (${configRows.length} units)...`);
            let configSheet = doc.sheetsByTitle['Oak Lodge Master Configs'];
            if (!configSheet) {
                configSheet = await doc.addSheet({
                    title: 'Oak Lodge Master Configs',
                    headerValues: ['UnitID', 'UnitName', 'Type', 'ContactNum', 'ElectricityMeter', 'WaterMeter']
                });
            }
            // Master configs are absolute snapshots of the building state override them all
